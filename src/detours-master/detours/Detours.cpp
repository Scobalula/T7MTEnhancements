...
namespace Detours
{
	...
	uint8_t *IATThunkHook(uint8_t *Module, PIMAGE_THUNK_DATA NameTable, PIMAGE_THUNK_DATA ImportTable, const char *API, uint8_t *Detour)
	{
		for (; NameTable->u1.Ordinal != 0; ++NameTable, ++ImportTable)
		{
			if (!IMAGE_SNAP_BY_ORDINAL(NameTable->u1.Ordinal))
			{
				auto importName = (PIMAGE_IMPORT_BY_NAME)(Module + NameTable->u1.ForwarderString);
	...
	uint8_t *IATHook(uint8_t *Module, const char *ImportModule, const char *API, uint8_t *Detour)
	{
		...
		for (size_t i = 0; importDescriptor[i].Name != 0; i++)
		{
			...
			if (!_stricmp(dllName, ImportModule))
			{
				...
				uint8_t *original = IATThunkHook(Module, nameTable, importTable, API, Detour);
	...
	uint8_t *IATDelayedHook(uint8_t *Module, const char *ImportModule, const char *API, uint8_t *Detour)
	{
		...
		// Get the load configuration section which holds the security cookie address
		auto dataDirectory = ntHeaders->OptionalHeader.DataDirectory;
		DWORD sectionRVA = dataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].VirtualAddress;
		DWORD sectionSize = dataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].Size;

		if (sectionRVA == 0 || sectionSize == 0)
			return nullptr;

		// Iterate over each delayed import descriptor
		auto importDescriptor = (PIMAGE_DELAYLOAD_DESCRIPTOR)(moduleBase + sectionRVA);

		for (size_t i = 0; importDescriptor[i].DllNameRVA != 0; i++)
		{
			PSTR dllName = (PSTR)(moduleBase + importDescriptor[i].DllNameRVA);

			// Is this the specific module the user wants?
			if (!_stricmp(dllName, ImportModule))
			{
				auto nameTable = (PIMAGE_THUNK_DATA)(moduleBase + importDescriptor[i].ImportNameTableRVA);
				auto importTable = (PIMAGE_THUNK_DATA)(moduleBase + importDescriptor[i].ImportAddressTableRVA);
